...
class AngrCFGError(Exception):
    pass


class AngrRuntimeError(Exception):
    pass
...
class SegmentList:
    ...
    def _insert_and_merge(self, address: int, size: int, sort: str, idx: int) -> None:
        ...
        if idx > 0 and address + size <= self._list[idx - 1].start:
            ...
            l.warning(
                "BUG FOUND: new segment should always be greater than _list[idx]."
            )
            ...
            if segment.sort == previous_segment.sort:
                ...
                if segment.start == previous_segment.end:
                    ...
                    if segment.start < previous_segment.start:
                        new_segments.append(
                            Segment(segment.start, previous_segment.start, segment.sort)
                        )

                        sort = (
                            previous_segment.sort
                            if direction == "forward"
                            else segment.sort
                        )
                        new_segments.append(
                            Segment(previous_segment.start, previous_segment.end, sort)
                        )
                        ...
                        if segment.end < previous_segment.end:
                            ...
                            new_segments.append(
                                Segment(previous_segment.end, segment.end, segment.sort)
                            )
                        ...
                        if segment.end > previous_segment.end:
                            new_segments.append(
                                Segment(segment.start, previous_segment.end, sort)
                            )
                            new_segments.append(
                                Segment(previous_segment.end, segment.end, segment.sort)
                            )
                            ...
                            new_segments.append(
                                Segment(segment.start, segment.end, sort)
                            )
                            new_segments.append(
                                Segment(
                                    segment.end,
                    ...
                    while len(new_segments) > 1 and i < len(new_segments) - 1:
                        ...
                        if s0.sort == s1.sort:
                            new_segments = (
                                new_segments[:i]
                                + [Segment(s0.start, s1.end, s0.sort)]
                                + new_segments[i + 2 :]
                            )
                        else:
                            i += 1
                    ...
                    old_size = sum(
                        seg.size
                        for seg in self._list[previous_segment_pos : segment_pos + 1]
                    )
                    ...
                    if len(new_segments) == 2:
                        ...
                        self._list = (
                            self._list[:previous_segment_pos]
                            + new_segments
        ...
        while idx < len(self._list):
            ...
            if segment.start <= address:
                if address < segment.start + segment.size < address + size:
                    ...
                    and address + size <= segment.start + segment.size
                ):
                    # |--------segment--------|
                    #    |--address + size--|
                    # break segment
                    ...
                    seg1 = Segment(
                        address + size, segment.start + segment.size, segment.sort
                    )
    ...
    def next_free_pos(self, address):
        ...
        if (
            idx < len(self._list)
            and self._list[idx].start <= address < self._list[idx].end
        ):
            # Occupied
            i = idx
            while (
                i + 1 < len(self._list) and self._list[i].end == self._list[i + 1].start
            ):
                i += 1
            if i == len(self._list):
        ...
        if idx < list_length:
            ...
            while i < list_length:
                if (
                    ...
                    and address + max_distance < self._list[i].start
                ):
                    return None
